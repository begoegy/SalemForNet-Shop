...
type CartItem = { name: string; amount_cents: number; quantity: number; description?: string; };
type BillingData = {
  first_name: string; last_name: string; email: string; phone_number: string;
  apartment: string; floor: string; street: string; building: string; city: string;
  country: string; state: string; postal_code?: string;
};

export async function POST(req: NextRequest) {
  try {
    const {
      amount_cents, currency = "EGP", merchant_order_id,
      items = [] as CartItem[], billing_data = {} as Partial<BillingData>,
      lock_order_when_paid = true,
    } = await req.json();

    const API_KEY = (process.env.PAYMOB_API_KEY ?? "").trim();   // 👈 قصقصة
    const INTEGRATION_ID = (process.env.PAYMOB_INTEGRATION_ID ?? "").trim();
    const IFRAME_ID = (process.env.PAYMOB_IFRAME_ID ?? "").trim();
    const ORIGIN = process.env.NEXT_PUBLIC_APP_URL || "https://www.salemfornet.art";
    ...
    const authRes = await fetch(`${BASE}/api/auth/tokens`, {
      ...
      headers: {
        ...
        "User-Agent": "Mozilla/5.0 (Windows NT 10.0; Win64; x64) SalemForNet/1.0",
        "Origin": ORIGIN,
        "Referer": ORIGIN + "/",
      },
    ...
    const orderRes = await fetch(`${BASE}/api/ecommerce/orders`, {
      ...
      body: JSON.stringify({
        auth_token, delivery_needed: false, amount_cents, currency,
        merchant_order_id: merchant_order_id ?? `SFN-${Date.now()}`,
        items: items.map((i) => ({
          name: i.name, amount_cents: i.amount_cents, quantity: i.quantity, description: i.description ?? "",
        })),
      }),
    });
    if (!orderRes.ok) {
    ...
    const payKeyRes = await fetch(`${BASE}/api/acceptance/payment_keys`, {
      ...
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({
        auth_token, amount_cents, currency, order_id, integration_id: Number(INTEGRATION_ID),
        lock_order_when_paid, billing_data: fullBilling,
      }),
    });
    if (!payKeyRes.ok) {
    ...
    // 4) IFRAME URL
    const iframeUrl = `${BASE}/api/acceptance/iframes/${encodeURIComponent(IFRAME_ID)}?payment_token=${encodeURIComponent(payment_token)}`;
