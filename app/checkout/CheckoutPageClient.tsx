...
export default function CheckoutPageClient() {
  ...
  async function submit() {
    try {
      ...
      // أنشئ الطلب في Firestore (لو متفعل)
      if (firebaseEnabled) {
        const payload = {
          uid: user.uid,
          email: user.email ?? "",
        ...
        setOrderRef(ref.id);
        setStatus("success");
        // ما نفضّيش السلة غير بعد إتمام الدفع بنجاح لو حابب
      } else {
        // لو لسه ما فعّلتش Firebase
      ...
      <div className="card p-4">
        ...
        {rows.length === 0 ? (
          ...
          <>
            ...
            {/* زرار تأكيد الطلب (إنشاء سجل الطلب) */}
            <div className="mt-3 flex gap-3">
              ...
              >
                {loading ? "جارٍ التحقق..." : "تأكيد الطلب"}
              </button>
            ...
            {/* زرار الدفع أونلاين — يظهر بعد وجود إجمالي ومستخدم */}
            {provider === "paymob" && total > 0 && user && (
              <div className="mt-4">
                <PaymobPayButton
                  ...
                  lastName=""
                  merchantOrderId={orderRef && orderRef.length ? orderRef : `SFN-${Date.now()}`}
                />
              </div>
            )}
          </>
